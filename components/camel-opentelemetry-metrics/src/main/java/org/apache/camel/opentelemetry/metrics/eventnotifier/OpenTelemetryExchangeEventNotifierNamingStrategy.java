    ...
    default Attributes getAttributes(ExchangeEvent event, Endpoint endpoint, boolean isBaseEndpointURI) {
        ...
        if (routeId != null) {
            return Attributes.of(
                    AttributeKey.stringKey(CAMEL_CONTEXT_ATTRIBUTE), exchange.getContext().getName(),
                    AttributeKey.stringKey(KIND_ATTRIBUTE), KIND_EXCHANGE,
                    AttributeKey.stringKey(EVENT_TYPE_ATTRIBUTE), event.getClass().getSimpleName(),
                    AttributeKey.stringKey(ROUTE_ID_ATTRIBUTE), routeId,
                    AttributeKey.stringKey(ENDPOINT_NAME_ATTRIBUTE), uri,
                    AttributeKey.stringKey(FAILED_ATTRIBUTE), Boolean.toString(exchange.isFailed()));
        } else {
            return Attributes.of(
                    AttributeKey.stringKey(CAMEL_CONTEXT_ATTRIBUTE), exchange.getContext().getName(),
                    AttributeKey.stringKey(KIND_ATTRIBUTE), KIND_EXCHANGE,
