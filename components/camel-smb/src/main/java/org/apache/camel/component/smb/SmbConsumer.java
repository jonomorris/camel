...
public class SmbConsumer extends GenericFileConsumer<FileIdBothDirectoryInformation> {
    ...
    private boolean handleDirectory(
        ...
        GenericFile<FileIdBothDirectoryInformation> genericFile = asGenericFile(fullFilePath, file, getEndpoint().getCharset());
        if (endpoint.isRecursive() && depth < endpoint.getMaxDepth() && isValidFile(genericFile, true, files)) {
    ...
    private void handleFile(
        ...
        SmbFile genericFile = asGenericFile(fullFilePath, file, getEndpoint().getCharset());
        if (depth >= endpoint.getMinDepth() && isValidFile(genericFile, false, files)) {
    ...
    protected Exchange createExchange(GenericFile<FileIdBothDirectoryInformation> file) {
        ...
        if (file != null) {
            ...
            ((SmbFile) file).populateHeaders(exchange);
        }
        return exchange;
    }
    ...
    protected boolean isMatched(
            GenericFile<FileIdBothDirectoryInformation> file, String doneFileName, FileIdBothDirectoryInformation[] files) {
